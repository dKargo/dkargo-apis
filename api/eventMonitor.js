...
let RunProc = async function() {
    try {
        ...
        let service = await new web3.eth.Contract(abiService, process.argv[2]);
         ...
         */
        service.events.CompanyUnregistered({fromBlock: startblock}, async function(error) {
            if(error != null) {
                Log('ERROR', colors.red(`ERROR: ${error}`));
            }
        }).on('data', async (log) => {
         ...
         * @param id 주문번호
         */
        service.events.OrderCreated({fromBlock: startblock}, async function(error) {
            if(error != null) {
                Log('ERROR', colors.red(`ERROR: ${error}`));
            }
        }).on('data', async (log) => {
         ...
         */
        service.events.OrderTransferred({fromBlock: startblock}, async function(error) {
            if(error != null) {
                Log('ERROR', colors.red(`ERROR: ${error}`));
            }
        }).on('data', async (log) => {
         ...
         */
        service.events.IncentiveUpdated({fromBlock: startblock}, async function(error) {
            if(error != null) {
                Log('ERROR', colors.red(`ERROR: ${error}`));
            }
        }).on('data', async (log) => {
         ...
         */
        service.events.Settled({fromBlock: startblock}, async function(error) {
            if(error != null) {
                Log('ERROR', colors.red(`ERROR: ${error}`));
            }
        }).on('data', async (log) => {
